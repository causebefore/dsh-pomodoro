...
- 非 loopback 的远程浏览器仍可使用计时器，但会使用内置默认值，且不能写入持久化设置。
- 当前不提供系统通知或提示音；阶段完成仅在面板内提示。
- 当前界面文案和设置分节名称仅提供简体中文，不随宿主 locale 切换。
