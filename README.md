...
- **双层完成提醒：** 阶段结束时显示 DSH 内全局提醒；可选在页面处于后台时发送浏览器系统通知。
...
系统通知仅在 DSH 页面处于后台或失去焦点时发送，前台不会与 DSH 内提醒重复打扰。请通过 `localhost`、`127.0.0.1` 或 HTTPS 访问，并保持 DSH 页面打开；关闭页面后的可靠提醒不在当前版本支持范围内。
