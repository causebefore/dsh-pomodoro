...
export function apply(ctx, config) {
  ...
  const settingsView = () => {
    ...
    return {
      ...
      writable: current.binding.provider.writable,
    };
  };
  ...
  ctx.connection.rpc.handle("/pomodoro", async (endpoint, payload) => {
    ...
    if (endpoint === "settings.save") {
      ...
      const next = parseSettingsValue(payload.value);
      if (next === null) return invalidSettingsValue();
      try {
