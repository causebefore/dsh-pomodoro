...
export function apply(ctx, config) {
  ...
  const activeSettings = () => {
    ...
    const descriptor = binding.provider.describe().find((item) => item.ns === SETTINGS_NAMESPACE);
  ...
  ctx.connection.rpc.handle("/pomodoro", async (endpoint, payload) => {
    ...
    if (endpoint === "settings.save") {
      ...
      await current.binding.scope.update({
    ...
    if (endpoint === "settings.reset") {
      const current = activeSettings();
      if (current === null) return settingsUnavailable();
      await current.binding.scope.replace({});
      return { ok: true, value: settingsView() };
    }
    if (endpoint !== "config") throw new Error(`dsh-pomodoro: 未知端点 ${JSON.stringify(endpoint)}`);
    const current = activeSettings();
    const resolved = current?.binding.scope.get() ?? entry;
    return {
      ok: true,
      value: {
        focusMs: Math.round(resolved.focusMinutes * 60 * 1000),
        breakMs: Math.round(resolved.breakMinutes * 60 * 1000),
        autoStartBreaks: resolved.autoStartBreaks,
        autoStartFocus: resolved.autoStartFocus,
      },
    };
  }, { authority: "loopback" });
}
