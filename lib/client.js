...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 双重身份常量：既是 locale 词典命名空间，也是 rc.7 keyed slot 的设置卡片 key。
    // 后者必须与 lib/index.js 的 SETTINGS_NAMESPACE 保持同值——DSH rc.7 按
    // entryKey=设置命名空间渲染设置卡片，不一致时卡片会静默消失（不报错）。
...
`;
    ...
    function apply(ctx) {
      ...
      // 设置表单走 DSH 官方 settingsScope，共享 settings.describe 镜像、revision 围栏与失效订阅。
      // 降级通道只读当前有效配置：settings provider 缺席时仍把宿主解析配置交给计时引擎。
      // 0.1.2+ 走 GET /api/pomodoro/config，≤0.1.1 走 /pomodoro RPC（见 callRuntimeConfig）。
      const connection = ctx.get("connection");
      const settingsScope = ctx.get("settingsScope").bind({ namespace: POMODORO_LOCALE_NS });
      ...
      const editableSettings = (expectedRevision) => {
        const current = settingsScope.getSnapshot();
      ...
      const pomoSettingsScope = {
        ...
        save: async (value, expectedRevision) => {
          editableSettings(expectedRevision);
        ...
        reset: async (expectedRevision) => {
          const initial = editableSettings(expectedRevision);
      ...
      ctx.effect(() => {
        syncSettingsConfig();
        return settingsScope.subscribe(syncSettingsConfig);
      }, "dsh-pomodoro: official settings scope");
    ...
    exports.inject = ["timer", "slots", "locale", "connection", "settingsScope"];
