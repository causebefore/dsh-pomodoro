    ...
    function apply(ctx) {
      ...
      function PomodoroSettings() {
        ...
        if (snap.status === "error") {
          return react.createElement(
            ...
            react.createElement(
              ...
              "设置加载失败：" + (snap.error?.message ?? String(snap.error)),
            ),
            react.createElement("button", {
              className: "pomo-btn",
              type: "button",
        ...
        if (snap.writable === false) {
          return react.createElement("div", { className: "pomo-setmsg", role: "status" }, "当前设置为只读；番茄钟仍使用宿主解析配置，但不能在这里持久化修改。");
        }
        ...
        const save = () => {
          ...
          if (f === null || b === null) {
            setNotice({ kind: "error", text: `请输入 ${MIN_DURATION_MINUTES}–${MAX_DURATION_MINUTES} 的整数分钟数` });
            return;
          }
          setBusy(true);
          setNotice(null);
