...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const pomoCss = `
.pomo-panel { position: fixed; right: 24px; bottom: 24px; width: 244px; z-index: 1000; pointer-events: auto; background: var(--dsw-alias-bg-overlay); color: var(--dsw-alias-label-primary); border: 1px solid var(--dsw-alias-border-l1); border-radius: 14px; box-shadow: var(--dsw-shadow-lv3); font-family: var(--dsw-font-family); overflow: hidden; user-select: none; touch-action: none; }
...
.pomo-title { font: var(--dsw-font-xs-strong-13); color: var(--dsw-alias-label-secondary); }
...
.pomo-flash { margin: 10px 12px 0; padding: 8px 10px; border-radius: 8px; background: var(--dsw-alias-state-business-tertiary); color: var(--dsw-alias-state-business-primary); font: var(--dsw-font-xxs-12); text-align: center; animation: pomo-pop 0.3s ease-out; }
...
@media (prefers-reduced-motion: reduce) { .pomo-flash { animation: none; } .pomo-ring-fg { transition: none; } }
`;
    ...
    function apply(ctx) {
      ...
      function PomodoroPanel() {
        const st = useEngine();
        const [, refreshFlash] = react.useState(0);
        react.useEffect(() => {
          if (st.flashAt === 0) return undefined;
          const remaining = 2600 - (Date.now() - st.flashAt);
          if (remaining <= 0) return undefined;
          const timeout = window.setTimeout(() => refreshFlash(Date.now()), remaining);
        ...
        return react.createElement(
          "div",
          {
            ...
            className: "pomo-panel" + (pos !== null ? " pomo-panel-moved" : ""),
            style: pos,
            role: "region",
            "aria-label": "番茄钟",
          },
          ...
          flashOn ? react.createElement("div", { className: "pomo-flash", role: "status", "aria-live": "polite" }, flashText) : null,
          react.createElement(
            "div",
            { className: "pomo-body" },
            react.createElement(
      ...
      function PomodoroSettings() {
        ...
        if (pomoSettingsScope === null) {
          return react.createElement("div", { className: "pomo-setmsg", role: "status" }, "当前部署没有可用的 settings 域；番茄钟仍使用宿主解析配置，但不能持久化修改。");
        }
        ...
        const updateDraft = (patch) => {
          setDrafts(Object.assign({}, draft, patch));
          setNotice(null);
        };
        const readNum = (field) => {
        ...
        const save = () => {
          ...
          pomoSettingsScope.save({
            ...
            setBusy(false);
            setNotice({ kind: "success", text: "已保存，将在下一次阶段切换时生效" });
          }, (error) => {
            setBusy(false);
            setNotice({ kind: "error", text: "保存失败：" + (error?.message ?? String(error)) });
          });
        };
        const clearOverrides = () => {
          ...
          pomoSettingsScope.reset().then(() => {
            ...
            setBusy(false);
            setNotice({ kind: "error", text: "清除失败：" + (error?.message ?? String(error)) });
          });
        };
        return react.createElement(
          "div",
          ...
          react.createElement(
            ...
            react.createElement("button", { className: "pomo-btn pomo-btn-main", type: "button", disabled: busy, onClick: save }, busy ? "保存中…" : "保存"),
            react.createElement("button", { className: "pomo-btn", type: "button", disabled: busy, onClick: clearOverrides }, "清除自定义设置")
          ),
