...
- `main` 是远端默认分支，只负责发布；对外展示的 README、版本徽章与 Release 均以 `main` 为准。
- `dev` 是本地集成分支：功能、修复、重构、文档等日常修改在 `dev` 或从 `dev` 创建的主题分支上进行。主题分支（`feat/xxx`、`fix/xxx`、`test/xxx`）完成后合并回本地 `dev`，不逐次推送远端；`dev` 在一批修改自检通过后统一 `git push`，远端 `dev` 仅作低频同步与云端备份。
- 单关注点的小改动可直接提交 `dev`；多步、跨文件或需要阶段性验证的改动必须先切主题分支，验证命令与手测场景写入合并提交的说明（单人项目的分支定位是自审与验证记录，不是协作门槛）。
- 开始修改前先检查当前分支；如果位于 `main`，应在工作区干净时切换到 `dev`，不得把日常开发提交直接留在 `main`。
- 只有完成发布检查、准备正式发布时，才通过发布 PR 将 `dev` 合入 `main`；除非用户明确授权执行发布，代理不得直接提交或推送 `main`。发布完成后继续回到 `dev` 开发。

## 提交、PR 与发布

提交采用 Conventional Commit，例如 `fix: 修正阶段提示消失时机`。每个提交只处理一个关注点。主题分支合回 `dev` 时，合并提交说明需列出验证命令与手测场景；UI 变化附明暗主题截图，peer 或 DSH 契约变化单独说明。发布前核对 README、版本、许可证、`npm pack` 文件清单和干净工作区，并先把 `dev` 推送远端。发布合并一律经发布 PR 在服务端完成：`gh pr create --base main --head dev` 后 `gh pr merge --merge`，禁止本地 merge 后直推 `main`（服务端合并不产生 push 事件且留下 PR 记录）；随后本地 `git pull` 同步两分支，再从 `main` 创建 Release 触发发布；Release 说明必须注明该版本适配（已验证）的 DSH 版本。除非用户明确授权，代理不得执行真实 `npm publish`。不要提交凭据、`settings.yaml`、日志或 `.tgz`。
